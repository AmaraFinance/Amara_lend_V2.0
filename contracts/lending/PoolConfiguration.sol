...
contract PoolConfiguration is IPoolConfiguration, Ownable {
  ...
  uint256 public maxBorrowInUSD;

  event SetMaxBorrowInUSD(uint256 maxBorrowInUSD);
  event SetCollateralPercent(uint256 collateralPercent);
  event SetConfig(uint256 baseBorrowRate,
    uint256 rateSlope1,
    uint256 rateSlope2,
  ...
  constructor() public {
    baseBorrowRate = 0.1 * 1e18;
    rateSlope1 = 0.2 * 1e18;
    rateSlope2 = 0.4 * 1e18;
    collateralPercent = 0.75 * 1e18;
    liquidationBonusPercent = 1.05 * 1e18;
    optimalUtilizationRate = 0.8 * 1e18;
    excessUtilizationRate = 0.2 * 1e18;
  }
  function setConfig(
     ...
     emit SetConfig(baseBorrowRate, rateSlope1, rateSlope2,
      collateralPercent, liquidationBonusPercent,
      optimalUtilizationRate, excessUtilizationRate);
   }

  function setCollateralPercent(uint256 _collateralPercent) public onlyOwner {
    collateralPercent = _collateralPercent;
    emit SetCollateralPercent(collateralPercent);
  }

  function setMaxBorrowInUSD(uint256 _maxBorrowInUSD) public onlyOwner {
    maxBorrowInUSD = _maxBorrowInUSD;
    emit SetMaxBorrowInUSD(maxBorrowInUSD);
  }

  function getMaxBorrowInUSD() external override(IPoolConfiguration) view returns (uint256) {
    return maxBorrowInUSD;
  }
