...
contract LendingPool is Ownable, ILendingPool, IReceiver, ReentrancyGuard {
   ...
   * @param maTokenAddress the address of the pool's maToken
   * @param poolConfigAddress the address of the pool's configuration contract
   */
  event PoolInitialized(
    address indexed pool,
    address indexed maTokenAddress,
   ...
   * Note: depositShares is the same as number of Token
  ...
  /**
   * @dev emitted on withdraw maToken
   * @param pool the address of the ERC20 token of the pool
   ...
   * @param collateralShares the amount of collateral shares which liquidator received from liquidation in from of maToken
  ...
  struct UserPoolData {
    ...
    uint256 borrowShares;
    // latest multiplier (borrow reward multiplier) of the user of this pool. Using to calculate current borrow reward.
    uint256 latestMultiplier;
    uint256 latestTokenMultiplier;
  }
  ...
  struct Pool {
    ...
    // total token reward on this pool
    uint256 totalMaraReward;
    uint256 totalTokenReward;
    //  reward multiplier of each borrow share
    uint256 multiplier;
  ...
  /**
   * @dev list of lending pools of mara.
   */
//  mapping(address => mapping(address => bool)) public authorizedBorrow;
  ...
  /**
   * @dev  token address contract.
   */
  ILendingPoolInfo public override lendingPoolInfo;

  /**
   * @dev MatokenDeployer address
   */
  MaTokenDeployer public maTokenDeployer;
  ...
  uint256 public liquidationPercent = 0.9 * 1e18;
  uint256 public reservePercent = 0.05 * 1e18;

  constructor(MaTokenDeployer _maTokenDeployer) public {
    maTokenDeployer = _maTokenDeployer;
  }

  /**
   * @dev setPoolStatus
   * @param _maTokenDeployer MaTokenDeployer
   */
  function setMaTokenDeployer(MaTokenDeployer _maTokenDeployer) external onlyOwner {
    maTokenDeployer = _maTokenDeployer;
  }
  ...
  /**
   * @dev update reward by call poke on distribution contract.
   */
  modifier updateReward() {
    if (address(lendingPoolInfo) != address(0)) {
      lendingPoolInfo.distributeMara();
    }
    _;
  }

//  function addAuthorizedBorrow(address _mara, address _token, bool _canBorrow) public onlyOwner {
//    authorizedBorrow[_mara][_token] = _canBorrow;
//  }
  ...
  function initPool(ERC20 _token, IPoolConfiguration _poolConfig) external onlyOwner {
    ...
    MaToken maToken = maTokenDeployer.createNewMaToken(maTokenName, maTokenSymbol, _token, address(msg.sender));
    ...
    emit PoolInitialized(address(_token), address(maToken), address(_poolConfig));
  }
   ...
   */
  function setPoolStatus(ERC20 _token, PoolStatus _status, bool _ableBorrow) external onlyOwner {
    Pool storage pool = pools[address(_token)];
    pool.status = _status;
    pool.ableBorrow = _ableBorrow;
  }
   ...
   * @param _token the ERC20 token of the pool
   * @return status - the pool's status, maTokenAddress - the pool's maToken, poolConfigAddress - the pool's configuration contract,
  ...
  {
    Pool storage pool = pools[address(_token)];
    uint256 userLiquidityShares = pool.maToken.balanceOf(_user);
    ...
    returns (uint256)
  {
    ...
    uint256 totalLiquidityShares = pool.maToken.totalSupply();
  ...
  function isAccountHealthy(address _user) public override view returns (bool) {
    ...
    return totalBorrowBalanceBase <= totalCollateralBalanceBase.wadMul(liquidationPercent);
  }
   ...
   * User can call this function to deposit their ERC20 token to the pool. user will receive the maToken of that ERC20 token
   ...
   * User will receive the liquidity shares in the form of maToken so Alice will have 5 alHello on her wallet
    ...
    updateReward
  {
    ...
    if(maxBorrowInUSD > 0){
      require(pool.totalBorrows <= maxBorrowInUSD, "Greater than the maximum of the pool that can be borrowed");
    }
   ...
   * @param _token the ERC20 token of the pool that user want to borrow
   * @param _amount the borrow amount
   * @param _user the user who wants to borrow. If _user is not msg.sender, sender must be mara lending pool
   * User can call this function to borrow the ERC20 token from the pool. This function will
   * convert the borrow amount to the borrow shares then accumulate borrow shares of this user
   * of this ERC20 pool then set to user data on that pool state.
   * e.g. Bob borrows 10 Hello tokens from the Hello token pool.
   ...
   * set Bob's borrow shares state with 2 borrow shares. Bob will receive 10 Hello tokens.
   */
  // function borrow(ERC20 _token, uint256 _amount, address _user)
  // external
  // nonReentrant
  // updatePoolWithInterestsAndTimestamp(_token)
  // updateReward
  // {
  //   require(authorizedBorrow[msg.sender][address(_token)], "not authorized to borrow");
  //   Pool storage pool = pools[address(_token)];
  ...
  //   // 5. check account health. this transaction will revert if the account of this user is not healthy
  //   require(isAccountHealthy(_user), "account is not healthy. can't borrow");
  //   emit Borrow(address(_token), _user, borrowShare, _amount);
  // }
   ...
   * @param _share the maToken amount that user want to withdraw
   * When user withdraw their liquidity shares or maToken, they will receive the ERC20 token from the pool
  ...
  /**
   * @dev set LiquidationPercent
   * @param _liquidationPercent the percent of liquidation
   */
  ...
  /**
    @dev set LendingPoolInfo address
   */
  function setLendingPoolInfo(ILendingPoolInfo _lendingPoolInfo) public onlyOwner {
    lendingPoolInfo = _lendingPoolInfo;
  }

  /**
   * @dev implement function of IReceiver interface to
   * receive token rewards from the LendingPoolInfo
   * @param _amount the amount of token to receive
   */
  function receiveToken(uint256 _amount) external override {
    require(msg.sender == address(lendingPoolInfo), "Only LendingPoolInfo can call receive Token");
    ...
    // This contract should not receive token if no borrow value lock in.
    if (totalBorrow == 0) {
      return;
    }
    lendingPoolInfo.mara().transferFrom(msg.sender, address(this), _amount);
  ...
  /**
   * @dev claim rewards(MARA and reward token) from unique ERC20 token pool and create receipt for caller
   */
  function claim(address _token) external updateReward nonReentrant {
    Pool storage pool = pools[_token];

    // claim rewards as a lender
    pool.maToken.claimCurrentRewardByLendingPool(msg.sender);

    // claim reward as a borrower
    claimCurrentReward(ERC20(_token), msg.sender);
  }

  /**
   * @dev update rewards for the borrower of the ERC20 pool
   * @param _pool the ERC20 token pool to update the rewards
   * @param _amount the total amount of the rewards to all borrowers of the pool
   */
  function updateBorrowReward(Pool storage _pool, uint256 _amount, uint256 _amount2) internal {
    _pool.totalMaraReward = _pool.totalMaraReward.add(_amount);
    _pool.totalTokenReward = _pool.totalTokenReward.add(_amount2);
    if (_pool.totalBorrowShares == 0) {
      return;
    }
    _pool.multiplier = _pool.multiplier.add(
      _amount.mul(1e12).div(_pool.totalBorrowShares)
    );
    _pool.multiplierToken = _pool.multiplierToken.add(
      _amount2.mul(1e12).div(_pool.totalBorrowShares)
    );
  }

  /**
   * @dev split the rewards between the lenders and borrowers
   * @param _token the ERC20 token pool
   * @param _amount the amount of token rewards to split
  ...
  function calculateReward(ERC20 _token, address _account) public view returns (uint256) {
    Pool storage pool = pools[address(_token)];
  ...
  function calculateTokenReward(ERC20 _token, address _account) public view returns (uint256) {
    ...
    uint256 pending = pool
    .multiplierToken
    .sub(userData.latestTokenMultiplier)
    .mul(userData.borrowShares)
    .div(1e12);
    return pending < pool.totalTokenReward ? pending : pool.totalTokenReward;
  }
  /**
   * @dev claim tokens rewards
   * @param _token the ERC20 pool
   * @param _account the user account that will claim the tokens
   */
  function claimCurrentReward(ERC20 _token, address _account) internal {
    ...
    Pool storage pool = pools[address(_token)];
    UserPoolData storage userData = userPoolData[_account][address(_token)];
    uint256 reward = calculateReward(_token, _account);
    uint256 tokenReward = calculateTokenReward(_token, _account);
    pool.totalMaraReward = pool.totalMaraReward.sub(reward);
    pool.totalTokenReward = pool.totalTokenReward.sub(tokenReward);
    userData.latestMultiplier = pool.multiplier;
    userData.latestTokenMultiplier = pool.multiplierToken;
    sendReward(_account, reward);
    pool.maToken.sendBorrowReward(_account, tokenReward);
  }
   ...
   */
  function sendReward(address _recipient, uint256 _amount) internal {
    uint256 bal = lendingPoolInfo.mara().balanceOf(address(this));
    if (bal > 0 && _amount > 0) {
      _amount = _amount < bal ? _amount : bal;
      lendingPoolInfo.mara().transfer(_recipient, _amount);
    }
  }
}
